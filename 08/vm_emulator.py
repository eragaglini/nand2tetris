...
def main():
    ...
    if os.path.isfile(path):
        ...
        onlyfiles = [os.path.join(path,f) for f in os.listdir(path) if os.path.isfile(os.path.join(path, f)) and f.endswith('.vm')]
        for f_path in onlyfiles:
            commands += parse_file(f_path)
        file_name = sys.argv[1] + '/' + sys.argv[1].split('.vm', 1)[0].split('/')[1] + '.asm'
