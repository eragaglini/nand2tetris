...
def write_push_pop(command):
    ...
    if command["command_type"] == CommandType.C_POP:
        if command["arg1"] in memory_segments_map.keys():
            line = "@{1}\nD=A\n@{0}\nA=M\nD=D+A\n@{0}\nM=D\n{2}\n@{0}\nA=M\nM=D\n@{1}\nD=A\n@{0}\nA=M\nD=A-D\n@{0}\nM=D".format(memory_segments_map[command["arg1"]], command["arg2"], pop_from_stack())
        else:
            if command["arg1"] == "temp":    
                line = "{0}\n@{1}\nM=D".format(pop_from_stack(),str(command["arg2"]+5))
            elif command["arg1"] == "static":
                name = sys.argv[1].split('.vm', 1)[0].split("/")[-1] + '.' + str(command["arg2"])
    ...
    # push operations
        ...
        if command["arg1"] == "constant":
            ...
            line = "@{1}\nD=A\n@{0}\nA=M\nD=D+A\nA=D\n{2}".format(memory_segments_map[command["arg1"]], command["arg2"],push_to_stack())
        else:
            if command["arg1"] == "temp": 
...
def compare_operation_value(opcode):
    ...
    line = "@TRUE{0}\nD;{1}\n@SP\nA=M\nM=0\n@END{0}\n0;JMP\n\n(TRUE{0})\n@SP\nA=M\nM=-1\n\n(END{0})\n".format(str(compare_op_num), compare_op_dict[opcode])
    compare_op_num += 1
    return line
...
def write_arithmetic(command):
    ...
    if command["arg1"] not in ["not", "neg"]:
        line = "@SP\nA=M\nA=A-1\nA=A-1\nD=M\nA=A+1\n{0}\n@SP\nM=M-1\nM=M-1\n{1}@SP\nM=M+1".format(opcode, updated_stack_value)
    else:
        line = "@SP\nM=M-1\nA=M\n{0}\n@SP\nM=M+1".format(opcode)
    return line
...
def write_function(command):
    ...
    for i in range(1,command["arg2"]):
        command_str += write_push_pop({"command_type": CommandType.C_PUSH, "arg1": "constant", "arg2": 0})
        command_str += "\n"
    return command_str
...
def write_function_return(command):
    command_str = create_frame_variable() + create_return_address() \
        + write_push_pop({"command_type": CommandType.C_PUSH, "arg1": "argument", "arg2": 0} ) \
        + reset_stack_pointer()
...
def write_function_call(command):
    ...
    for i in [return_label, 'LCL', 'ARG', 'THIS', 'THAT']:
        command_str += write_push_pop({'command_type': CommandType.C_PUSH,'arg1': 'constant', 'arg2': i}) + '\n\n'
...
def write_sys_init():
    return ["@256\nD=A\n@SP\nM=D", write_function_call({'command_type': CommandType.C_CALL, 'arg1': 'Sys.init', 'arg2': 0})]
